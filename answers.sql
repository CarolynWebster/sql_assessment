...
SELECT m.name, b.name, b.headquarters FROM models AS m JOIN brands AS b USING (brand_id) WHERE b.name = 'Ford' AND m.name = 'Mustang';
...
SELECT m.year, m.name, m.model_id, a.name FROM models AS m LEFT JOIN awards AS a ON m.model_id = a.winner_id WHERE m.year >= 1964 ORDER BY m.year desc;
...
SELECT b.name, m.year, m.name FROM brands AS b JOIN models AS m USING (brand_id) ORDER BY b.name;
...
FROM models AS m LEFT JOIN brands AS b USING (brand_id) WHERE b.discontinued IS NOT NULL
...
SELECT b.name, b.founded FROM brands AS b LEFT JOIN models AS m USING (brand_id) WHERE m.name IS null;
...
SELECT m.name, m.year FROM brands AS b JOIN models AS m USING (brand_id) WHERE m.year IN (SELECT founded FROM brands);